...
public class serverGUI {
    ...
    void initialize() {
        ...
        try {
            ...
            input_logs.setText(input_logs.getText()+"Loaded information to input fields\n");
    ...
    void mysql_connection_test_button(MouseEvent event) {
        if(input_host.getText().isEmpty() || input_username.getText().isEmpty() || input_db_name.getText().isEmpty() || input_mysql_port.getText().isEmpty())
        {
            input_logs.appendText("there is empty field/s related to mysql\n");
            return;
        }

        save_information_to_file();

        mysqlConnection connection = new mysqlConnection(input_host.getText(), input_username.getText(), input_password.getText(), input_db_name.getText(), input_mysql_port.getText());
        connection.connectToDB();
        input_logs.setText(input_logs.getText()+connection.getMessage());
        connection.disconnetFromDB();
        input_logs.setText(input_logs.getText()+connection.getMessage());
    ...
    void server_connection_test_button(MouseEvent event) {
        ...
        {
            input_logs.setText(input_logs.getText()+"Port number field left empty\n");
            return;
        }
        else if(!isStringInt(input_server_port.getText()))
        {
            input_logs.setText(input_logs.getText()+"The port number contains invalid letters\n");
            return;
        }
        else if( 0< Integer.parseInt(input_server_port.getText()) && Integer.parseInt(input_server_port.getText()) > 9999)
        {
            input_logs.setText(input_logs.getText()+"The port number less than 10,000\n");
            return;
        }

        save_information_to_file(); // save the input into file

        CEMSServer server = new CEMSServer(Integer.parseInt(input_server_port.getText()));
        server.Start();
    ...
    private void logs_scroll()
    {
        Thread thread = new Thread(){
            public void run(){
                ...
                {
                    input_logs.setScrollTop(Double.MAX_VALUE);
                    scrollPosition = input_logs.getScrollTop();
                }
                else
                    input_logs.setScrollTop(scrollPosition);
            }
          };
        thread.start();
    }
   ...
   private void save_information_to_file()
    {
        /*if( ! host.equals(input_host.getText()) || ! username.equals(input_username.getText()) || ! password.equals(input_password.getText()) ||
         ! db_name.equals(input_db_name.getText()) || ! mysql_port.equals(input_mysql_port.getText()) || ! server_port.equals(input_server_port.getText()) ||
        ...
        {*/
            ...
            try {
                ...
                input_logs.setText(input_logs.getText()+"Saved inputs to file\n");
            } catch (IOException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
    ...
    void start_server_button(MouseEvent event) {
       ...
       // input_logs.setText(input_logs.getText()+connection.getMessage());
        CEMSServer server = new CEMSServer(Integer.parseInt(input_server_port.getText()),input_host.getText(), input_username.getText(), input_password.getText(), input_db_name.getText(), input_mysql_port.getText());
        server.Start();
       // Statement stmt;
      /*  try {
